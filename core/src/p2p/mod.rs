        ...
        match msg {
            Message::Handshake { version, agent, height, network, nonce } => {
                ...
                if nonce != 0 && nonce == node_nonce() {
                    println!("[P2P] Self-connection detected (nonce match) from {} — disconnecting", scalar_addr);
                ...
                if network.is_empty() {
                    println!("[P2P] Rejecting peer {} — empty network magic (pre-v2.9.5 node?)", scalar_addr);
                    return Err(anyhow::anyhow!("Empty network magic — handshake rejected"));
                }
                if network != my_net.magic() {
                    println!("[P2P] Rejecting peer {} — network mismatch: {} vs {}", scalar_addr, network, my_net.magic());
                ...
                if height > my_h {
                    ...
                    if sync_status.should_enter_ibd(my_h, height) {
                        ...
                        println!("📥 IBD: requesting blocks {}-{} from {}", my_h + 1, my_h + sync::IBD_BATCH_SIZE as u64, scalar_addr);
                        ...
                        println!("[P2P] Skipping sync from {} — IBD already in progress", scalar_addr);
                    }
                }
            }
            ...
            Message::GetBlocksIBD { from_height, limit } => {
                 ...
                 if count > 0 {
                     println!("📤 IBD: Served {} blocks (from={}, remaining={})", count, from_height, remaining);
                 }
            }
            ...
            Message::Blocks { blocks } => {
                ...
                if state.reorging.load(std::sync::atomic::Ordering::Relaxed) {
                    println!("[P2P] Ignoring {} blocks from {} — reorg in progress", blocks.len(), scalar_addr);
                    continue;
                }
                ...
                for b in &blocks {
                    if let Err(e) = state.process_block(b.clone()) {
                        if e.contains("Invalid prev_hash") || e.contains("Previous block") {
                            ...
                            fork_detected = true;
                            println!("🔀 Fork detected from {} at height {} — attempting reorg", scalar_addr, b.height());
                            break;
                        }
                ...
                if fork_detected {
                    ...
                    match reorg::find_fork_point(&state.storage, &blocks) {
                        Ok(fork_point) => {
                            ...
                            println!("🔀 Fork point at height {}, our tip={}, peer tip={}",
                            ...
                            println!("🔀 Requesting full fork chain: {} blocks from height {}",
            ...
            Message::BlocksIBD { blocks, remaining } => {
                ...
                if batch_len == 0 {
                            ...
                            Ok(fork_point) => {
                                ...
                                if fork_blocks.is_empty() || !contiguous {
                                    if !fork_blocks.is_empty() {
                                        ...
                                        println!("🔀 IBD fork: no blocks above fork_point {}, skipping", fork_point);
                                    }
                                    state.reorging.store(false, std::sync::atomic::Ordering::Relaxed);
                                } else {
                    ...
                    if remaining > 0 && processed > 0 {
                        ...
                        println!("⚠️ IBD: no blocks processed from batch, re-requesting from height {}", current + 1);
                        let _ = tx.send(Message::GetBlocksIBD {
